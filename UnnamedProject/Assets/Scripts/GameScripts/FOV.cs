...
public class FOV
{
	...
	int N = GlobalData.field.Count;
	int M = GlobalData.field[0].Count;
	...
	public void visibleCell(int x, int y)
	{
		...
		for (int nowX = 0; nowX <= trueX; nowX++)
		{
			...
			{
				if (getOpacityInCell(curX + nowX * signX, curY + nowY * signY))
				{
					inside--;
					if (inside == 0)
					{
				...
				{
					if (getOpacityInCell(curX + (prevX + 1) * signX, curY + prevY * signY) &&
						getOpacityInCell(curX + prevX * signX, curY + (prevY + 1) * signY))
					{
						inside--;
						if (inside == 0)
							return;
	...
	//TODO: Ссылки изнутри FOV на range и inside у юнита
